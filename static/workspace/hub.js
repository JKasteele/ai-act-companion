...
function card(s) {
  const a = s.answers;
  return `<a class="system-card" href="#system/${encodeURIComponent(s.id)}/overview"><div class="source-meta"><span>${s.source === "example" ? "Example system" : s.source === "server" ? "Saved in local app" : "Your system"}</span><span class="status-pill ${riskClass(s.result?.classification)}">${esc(s.result?.classification?.tier_label || "Draft")}</span></div><h3>${esc(a.sys_name)}</h3><p>${esc(a.intended_purpose || a.sys_description || "Complete the system profile to start the assessment.")}</p><div class="card-meta"><span>${esc(a.sys_owner || "Owner not assigned")}</span><span>Open workspace ↗</span></div></a>`;
}
function home() {
